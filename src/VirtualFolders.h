...
namespace jsIO
{
  class VirtualFolders{
    public:
      ...
      inline VirtualFolder& operator [] (const int& i){return m_vFolders[i];};

      std::vector<VirtualFolder> getFolders() { return m_vFolders ; }
      bool containPrecomputeExtents(){  return m_containsPrecomputedExtents ; }
      int count() const {return m_vFolders.size();}
      void setPreComputedExtents(std::vector<std::string> &_precomputedTraceExtents, std::vector<std::string> &_preComputedHeaderExtents);
