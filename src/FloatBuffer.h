...
namespace jsIO
{
  class FloatBuffer : public Buffer{
    public:
      ...
      void setBuffer(float* farray, unsigned long farraylen){setBufferBase((char*)farray,farraylen*SIZEOFFLOAT); buffer_pos=0;};
      void wrap(float* farray, unsigned long farraylen){wrapBase((char*)farray,farraylen*SIZEOFFLOAT); buffer_pos=0;};

      size_t size() const {return (size_t)(sizeBase()/SIZEOFFLOAT);};
      size_t capacity()const {return capacityBase()/SIZEOFFLOAT;};
      void resize(size_t sz){resize(sz*SIZEOFFLOAT);};
      void reserve(size_t sz){reserveBase(sz*SIZEOFFLOAT);};

      const float* array() const{return (const float*)(arrayBase());};
