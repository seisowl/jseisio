...
AxisDefinition::AxisDefinition(AxisLabel _label, Units _units, DataDomain _domain, long _length, long _logicalOrigin,
    long _logicalDelta, double _physicalOrigin, double _physicalDelta, std::string _headerName,
    std::string _headerBinName) {
  Init(_label, _units, _domain, _length, _logicalOrigin, _logicalDelta, _physicalOrigin, _physicalDelta, _headerName,
      _headerBinName);
}

void AxisDefinition::Init(AxisLabel _label, Units _units, DataDomain _domain, long _length, long _logicalOrigin,
    long _logicalDelta, double _physicalOrigin, double _physicalDelta, std::string _headerName,
    std::string _headerBinName) {
  label = _label;
  units = _units;
...
void AxisDefinition::getDefault(int ndim, int *idim, AxisDefinition *& axes) {
  ...
  for (int i = 0; i < ndim; i++) {
    axes[i].Init(labels[i], Units::UNDEFINED, DataDomain::UNDEFINED, (long) idim[i], 0, 1, 0.0, 1.0);
  }
}
...
int AxisDefinition::subRange(long* _range, int _rangelen, AxisDefinition *subAxis) {
  ...
  subAxis->Init(label, units, domain, _length, _logicalOrigin, _logicalDelta, _physicalOrigin, _physicalDelta);
  return JS_OK;
}
