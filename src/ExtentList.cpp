...
namespace jsIO
{
  ...
  ExtentList::ExtentList( std::string _extBaseName, int _numExtents, long _maxFilePosition, long _extentSize, VirtualFolders _vFolders) 
  {
    Init(_extBaseName, _numExtents, _maxFilePosition,  _extentSize,  _vFolders);
  }

  int ExtentList::Init( std::string _extBaseName, int _numExtents, long _maxFilePosition, long _extentSize, VirtualFolders _vFolders) 
  {
...
//     printf(" numVFolders=%d, numExtInEachFolder=%d, numExtInFirstFolder=%d\n",numVFolders, numExtInEachFolder, numExtInFirstFolder);
    ...
    for(int j=0;j<numExtInFirstFolder;j++)
    {
      ...
      TRACE_PRINTF(ExtentListLog, "Extent Info: Name=%s, Ind=%d, startOffset=%lu, Size=%lu, Path=%s",extName.c_str(), extInd, startOffset, extentSize, extPath.c_str());
...
//   if(numExtents>0) maxFilePosition = extents[numExtents-1].getStartOffset()+extents[numExtents-1].getExtentSize();
    ...
    TRACE_PRINTF(ExtentListLog,"List of %s Extents . Found: %d, Max:%d",extBaseName.c_str(),numExtentsFound,numExtents);
...
//   int numExt=0;
    ...
    if(parNumExt==0 || parExtSize==0 || parExtBaseName==0 || parMaxFilePos==0){
      ERROR_PRINTF(ExtentListLog, "Error in XML file %s. Some of these %s, %s, %s, %s tags are missing.", 
                   ExtentManagerXML.c_str(), sVFIO_EXTSIZE.c_str(), sVFIO_MAXFILE.c_str(), sVFIO_EXTNAME.c_str(), sVFIO_MAXPOS.c_str());
      return JS_USERERROR;
    } 
...
*/
 // ************ 
  ...
  long ExtentList::computeExtentSize(long fileLength, int numExtents, long frameSize) 
  {
    ...
    if (fileLength % numExtents != 0)
      extentCreateSize++;
    long fact = extentCreateSize / frameSize;
    if (extentCreateSize % frameSize != 0)
      fact = fact + 1;
...
//save to XML file
  int ExtentList::saveXML(std::string _path)
  {
    ...
    std::string VExtManXML =
        ...
        <par name=\"VFIO_EXTSIZE\" type=\"long\"> "+num2Str(extentSize)+" </par>\n\
        <par name=\"VFIO_MAXFILE\" type=\"int\"> "+num2Str(numExtents)+" </par>\n\
        <par name=\"VFIO_MAXPOS\" type=\"long\"> "+num2Str(maxFilePosition)+" </par>\n\
        <par name=\"VFIO_EXTNAME\" type=\"string\"> "+extBaseName+" </par>\n\
