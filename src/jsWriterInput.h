 ...
 */
  class jsWriterInput
  {
    ...
    public:
      ...
      */
      void setFileName(const std::string _filename){jsfilename = _filename;}
      
      void setFileDescription(const std::string _description){description = _description;}
      void setMapped(bool _mapped){isMapped=_mapped;}
      
      /**
       * @brief Set cache size used for writing to files. 
       * @details Default is 2MB. Set 0 to write directly (not cached).
       */
      void setDiskCacheSize(unsigned long _cacheSize){IOBufferSize = _cacheSize;}
      ...
      */
      void setNumberOfExtents(const int _numExtends){NExtends = _numExtends;}
      /**
      ...
      */
      void setSeispegPolicy(int _policy){seispegPolicy= _policy;}
       ...
       *                     This is not a requred parameter.
      */
      int initGridAxis(int _axisInd, AxisLabel _label, Units _units, DataDomain _domain,
      ...
      */
      ...
      void add_secondary_path(std::string _path){virtualFolders.push_back(_path);}
      ...
      */
      void addSurveyGeom(int i1, int i2, int i3, int i4,
