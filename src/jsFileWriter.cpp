...
//   fprintf(plogfile,"m_headerLengthBytes=%d, m_numTraces=%d, m_headerFileSize=%d\n",m_headerLengthBytes, m_numTraces, m_headerFileSize);
  ...
  TRACE_PRINTF(jsFileWriterLog, "traceLen=%d, TotalNumOfFrames=%ld, TotalNumOfTraces=%ld", m_compess_traceSize,
      m_TotalNumOfFrames, m_TotalNumOfTr);
...
int jsFileWriter::writeSingleProperty(std::string datasetPath, std::string fileName, std::string propertyName,
...
long jsFileWriter::getOffsetInExtents(int* indices, int len1d) {
  ...
  for (int i = 0; i < ind_len; i++) {
    if (indices[i] < 0 || indices[i] >= m_fileProps->axisLengths[ind_len - i]) {
      ERROR_PRINTF(jsFileWriterLog, "index %d must be positive and smaller than %ld", indices[i],
...
int jsFileWriter::writeFrames(int frameIndex, float *frames, int nFrames) {
  ...
  if (frameIndex < 0 || frameIndex + nFrames >= m_TotalNumOfFrames) {
    ERROR_PRINTF(jsFileWriterLog, "Invalid frame index. [%d,%d] must be in [0,%ld)", frameIndex, frameIndex + nFrames,
        m_TotalNumOfFrames);
    return JS_USERERROR;
  }
  ...
  //write only with native order
    ...
    if (m_bSeisPEG_data) {
      if (headbuf != NULL) {
        ...
        bytesInFrame = m_seispegCompressor->compress((float*) frame, numLiveTraces, m_seispegHeaderBuffer,
            m_headerLengthWords, m_traceBufferArray);
...
//       printf("---   i=%d, trc_type=%d, ifull=%d\n", i, trc_type, ifull);
    ...
    if (trc_type == 0) {
      ...
      memcpy(&tmpFrameBuf[ifull * m_numSamples], &frame[j * m_numSamples],
          (numTraces - j) * m_numSamples * sizeof(float));
      memcpy(&tmpHrdBuf[ifull * m_headerLengthBytes], &headbuf[j * m_headerLengthBytes],
          (numTraces - j) * m_headerLengthBytes);
    } else {
      ifull++;
    }
    i++;
...
char* jsFileWriter::allocHdrBuf(bool initVals) {
  ...
  {
    float TFULL_E = m_gridDef->getAxisPhysicalOrigin(0)
        + m_gridDef->getAxisLength(0) * m_gridDef->getAxisPhysicalDelta(0);
