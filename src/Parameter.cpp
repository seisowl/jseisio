...
std::string Parameter::getValuesAsString() const {
  ...
  if (N_values < 2) str = str.substr(3, str.length() - 2);
...
void Parameter::setTypeAsString(std::string stype) {
  if (stype == "boolean" || stype == "bool") type = BOOLEAN;
  else if (stype == "int") type = INT;
  else if (stype == "long") type = LONG;
  else if (stype == "float") type = FLOAT;
