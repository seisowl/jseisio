...
bool Parameter::setValues(std::string &_values, int _type) {
  ...
  if(_type == BOOLEAN) {
    ...
    if(N == 0 || k > 0) {
      ERROR_PRINTF(ParameterLog, "Type mismatch: %s can not be interpreted as an array of booleans.", _values.c_str());
      type = UNDEFINED;
      N_values = 0;
      return false;
    }
...
bool Parameter::valuesAsFloats(float *fval) const {
  if(type == FLOAT) {
    for(int i=0; i<N_values; i++) fval[i] = fvalues[i];
    return true;
  }
  if(type == DOUBLE) {
    for(int i=0; i<N_values; i++) fval[i] = (float) dvalues[i];
    return true;
  }
  if(type == INT) {
    for(int i=0; i<N_values; i++) fval[i] = (float) ivalues[i];
    return true;
  }
  if(type == LONG) {
    for(int i=0; i<N_values; i++) fval[i] = (float) lvalues[i];
    return true;
  }
  return false;
}
...
bool Parameter::valuesAsDoubles(double *dval) const {
  if(type == FLOAT) {
    for(int i=0; i<N_values; i++) dval[i] = fvalues[i];
    return true;
  }
  if(type == DOUBLE) {
    for(int i=0; i<N_values; i++) dval[i] = dvalues[i];
    return true;
  }
  if(type == INT) {
    for(int i=0; i<N_values; i++) dval[i] = (double) ivalues[i];
    return true;
  }
  if(type == LONG) {
    for(int i=0; i<N_values; i++) dval[i] = (double) lvalues[i];
    return true;
  }
  return false;
}
