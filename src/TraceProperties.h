...
namespace jsIO
{
  ...
  class TraceProperties {
    ...
    public: 
      ...
      void reserveBuffer(long bsize){buffer->reserve(bsize);};
      void resizeBuffer(long bsize){buffer->resize(bsize);};
...
//    int loadBuffer(std::string filename);
      ...
      unsigned long getBufferPos(){return buffer_pos;};
      unsigned long getBufferSize(){return buffer->size();};
      unsigned long getBufferCapacity(){return buffer->capacity();};
   ...
   //headbuf must be filled with the headers of numOfTraces traces
      ...
      std::vector<catalogedHdrEntry> getHdrEntries(){return hdrEntries;}
         
//    void setBufferPosition(PropertyDescription &property);
      void setTraceIndex(int _traceIndex) {traceIndex = _traceIndex;};
