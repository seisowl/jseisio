...
//   printf("NDir=%d\n",NDIR);
  ...
  //   not the full path but "project" path to which vFolderPathRest must be added to get the full path
    ...
    if (found!=std::string::npos) vFolderPathRest = jsDataPathTemp.substr(found, jsDataPathTemp.length()) + vFolderPathRest;
...
//Static delete method to remove the properties file.
  ...
  {
    std::string VirualFoldersXML = _path + JS_VIRTUAL_FOLDERS_XML;
    if( remove( VirualFoldersXML.c_str()) != 0 )
      return false;
    else
      return true;
  }
  ...
  */
  ...
  void VirtualFolders::setPreComputedExtents(std::vector<std::string> &_precomputedTraceExtents, std::vector<std::string> &_preComputedHeaderExtents)
  {
  ...
  int VirtualFolders::save(std::string _path)
  {
    ...
    if(numVF==1 && sFirstVF == _path){
      ...
      for(int i=0;i<numVF;i++){
        ...
        found = vFolder.find_last_of('/',found-1);
        vF += "  <par name=\"FILESYSTEM-"+num2Str(i)+"\" type=\"string\"> "+num2Str(vFolder.substr(0,found))+",READ_WRITE </par>\n";
      }
    }
    ...
    std::string VFoldersXML =
        ...
        <par name=\"GLOBAL_REQUIRED_FREE_SPACE\" type=\"long\"> "+num2Str(globalReqFreeSpace)+" </par>\n\
        </parset>\n";
...
//recursive mkdir
  int VirtualFolders::mkdirp(const char *dir, mode_t mode)
  {
    ...
    if(*(p-1) != '/')
      if(::mkdir(dir_cpy, mode) == -1 && errno != EEXIST)
    {
