...
int jsFileReader::Init(const std::string _jsfilename, const int _NThreads) {
  ...
  if (m_fileProps->isMapped == false) { //if not mapped, then it must be regular
    ...
    while (ifile.good() && numReadedInts < m_TotalNumOfFrames) {
      int ints2Read =
          (m_TotalNumOfFrames - numReadedInts > maxInt2read) ? maxInt2read : m_TotalNumOfFrames - numReadedInts;
...
//              TRACE_PRINTF(jsFileReaderLog, "Not Regular %d!=%d", iBuffer[i],m_numTraces);
  ...
  TRACE_PRINTF(jsFileReaderLog, "traceLen=%d, TotalNumOfFrames=%ld, TotalNumOfTraces=%ld", m_compess_traceSize,
      m_TotalNumOfFrames, m_TotalNumOfLiveTraces);
...
long jsFileReader::readTraces(const long _firstTraceIndex, const long _numOfTraces, float *buffer, char *headbuf) {
  ...
  if (_firstTraceIndex + _numOfTraces > m_TotalNumOfTraces) {
    ERROR_PRINTF(jsFileReaderLog,
        "Can't read %ld traces starting from the trace #%ld, because there are only %ld traces total", _numOfTraces,
        _firstTraceIndex, m_TotalNumOfTraces);
    return -1;
  }
...
//       printf("Read new frame %ld, nLiveTr=%d\n",frameInd, nLiveTr);
  ...
  if (headbuf) {
    memcpy((char*) &headbuf[posHeader], (char*) &frameHeader[trInd * m_headerLengthBytes],
        (numTraces * m_headerLengthBytes));
  }
...
int jsFileReader::liveToGlobalTraceIndex(const long _liveTraceIndex, long &_globalTraceIndex) {
  ...
  if (_liveTraceIndex < 0 || _liveTraceIndex >= m_TotalNumOfLiveTraces) {
    ERROR_PRINTF(jsFileReaderLog, "Invalid live trace index. %ld must be in [0,%ld)", _liveTraceIndex,
...
long jsFileReader::readWithinLiveTraces(const long _firstTraceIndex, const long _numOfTraces, float *buffer,
    char* headbuf) {
...
long jsFileReader::getOffsetInExtents(int* position, int len1d) const {
  ...
  for (int i = 0; i < ind_len; i++) {
    if (position[i] < 0 || position[i] >= m_fileProps->axisLengths[ind_len - i]) {
      ERROR_PRINTF(jsFileReaderLog, "index %d must be positive and smaller than %ld", position[i],
          m_fileProps->axisLengths[i]);
      return JS_USERERROR;
    }
    volsize = len1d;
...
int jsFileReader::readFrame(const long _frameIndex, float *frame, char *headbuf) {
  ...
  if (m_bIsFloat) //if float, there is no need to uncompress, we can read directly into frame (should be faster)
  {
    ...
    if (m_bSeisPEG_data) {
      if (headbuf != NULL) {
        m_seispegCompressor->uncompress(m_traceBufferArray, bytesInFrame, frame, numLiveTraces, m_headerBufferView,
            m_headerLengthBytes);
        m_traceProps->getTraceHeader(0, numLiveTraces, headbuf);
      } else {
...
long jsFileReader::getTraceIndex(const int* position) const // *position must be in logical corrdinates
  ...
  int numAxis = m_fileProps->numDimensions - 1;
  int pInd = numAxis - 1;
  int trInd = (position[pInd] - m_fileProps->logicalOrigins[numAxis - pInd])
      / m_fileProps->logicalDeltas[numAxis - pInd];
...
//       return JS_USERERROR;
  ...
  if (m_bIsFloat) { //if float, there is no need to uncompress, we can read directly into frame (should be faster)
    ...
    if (m_bSeisPEG_data) {
      if (headbuf != NULL) {
        m_seispegCompressor[iThread].uncompress(rawframe, m_frameSize, frame, numLiveTraces, (int*) headbuf,
            m_headerLengthWords);
...
//          TRACE_VAR(jsFileReaderLog,extInd);
      ...
      TRACE_PRINTF(jsFileReaderLog, "Open new file %d, %d, %s, %d", m_currIndexOfTrFileExtent, extInd, fname.c_str(),
          m_curr_trffd);
      if (m_curr_trffd < 0) {
        return JS_WARNING;
      }
...
int jsFileReader::readFrameHeader(const long _frameIndex, char *headbuf) {
  ...
  if (!m_bSeisPEG_data) {
    ...
    m_seispegCompressor->uncompressHdrs(m_traceBufferArray, bytesInFrame, (int*) m_headerBufferArray,
        m_headerLengthBytes);
    m_traceProps->getTraceHeader(0, numLiveTraces, headbuf);
  }
  return numLiveTraces;
