...
namespace jsIO
{
  class ExtentList{
    public:
      ~ExtentList(){}
      ExtentList():bInit(false){}
      ExtentList( std::string _extBaseName, int _numExtents, long _maxFilePosition, long _extentSize, VirtualFolders _vFolders);
      int Init ( std::string _extBaseName, int _numExtents, long _maxFilePosition, long _extentSize, VirtualFolders _vFolders);      
      int InitFromXML(const std::string &jsDataPath, std::string ExtentManagerXML);
      
      void setVirtualFolders(VirtualFolders &_vFolders){vFolders = _vFolders;}
      VirtualFolders getVirtualFolders(){return vFolders;}
      
      inline ExtentListEntry& operator [] (int i) {return extents[i];};
      
      std::vector <ExtentListEntry> getExtents() {return extents;};
      std::string getExtentBase() {return extBaseName;};
      void setExtentBase(std::string _extBaseName) {extBaseName=_extBaseName;};
      ...
      int getNumExtents() const {return numExtents;}
   
      long getSumOfExtSizes() const;
      int getNumvFolders() const{return vFolders.count();};
