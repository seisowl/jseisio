...
namespace jsIO
{
  class PropertyDescription{
    public:
      ...
      enum HDR_FORMAT {HDR_FORMAT_UNDEFINED = 0, HDR_FORMAT_BYTE = 1, HDR_FORMAT_SHORT = 2, 
        HDR_FORMAT_INTEGER = 3, HDR_FORMAT_LONG = 4, HDR_FORMAT_FLOAT=5, 
        HDR_FORMAT_DOUBLE=6, HDR_FORMAT_COMPLEX=7, HDR_FORMAT_DCOMPLEX=8, 
        HDR_FORMAT_STRING=9, HDR_FORMAT_BYTESTRING=10};
...
// private atributes
    ...
    public:
      ...
      std::string getFormatString() const {return(c_formatStrings[format]);}
      void setFormat(std::string _sformat);
      void setFormat(int _format){format=_format;};

      int getFormatLength() const {return(formatLength);}
      int getRecordLength() const {return(recordLength);}
