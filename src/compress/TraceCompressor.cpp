...
//       printf("length=%d, bytesPerSample=%d\n",length, bytesPerSample);
  ...
  int TraceCompressor::setPosition(int _tracePosition) {
    ...
    if(traceFormat.getName() == "SEISPEG"){
      ...
      bufferViewFloat.position(tracePosition * recordLengthInFloats);
    }else if(traceFormat.getName() == "INT16"){
      bufferViewShort.position(tracePosition * recordLengthInShorts);
    }else if(traceFormat.getName() == "INT08"){
      bufferViewByte.position(tracePosition * recordLengthInBytes);
    }else if(traceFormat.getName() == "COMPRESSED_INT16"){
      ...
      return JS_USERERROR;
                 ...
                 traceFormat.getName() == "INT08")
             {
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 packTrace(&_traceData[i*numSamples]);
               }
             }else if(traceFormat.getName() == "COMPRESSED_INT16"){
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 packTrace16(&_traceData[i*numSamples]);
               }
             }else if(traceFormat.getName() == "COMPRESSED_INT08"){
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 packTrace08(&_traceData[i*numSamples]);
               }
             }
    ...
    if(traceFormat.getName() == "SEISPEG"){
      ...
      return JS_USERERROR;
                 ...
                 traceFormat.getName() == "INT08")
             {
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 unpackTrace(&_traceData[i*numSamples]);
               }
             }else if(traceFormat.getName() == "COMPRESSED_INT16"){
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 unpackTrace16(&_traceData[i*numSamples]);
               }
             }else if(traceFormat.getName() == "COMPRESSED_INT08"){
               for(int i = 0; i < _numTraces; i++) {
                 setPosition(i);
                 unpackTrace08(&_traceData[i*numSamples]);
               }
             }
...
//  testfuction
  void testTraceCompressor()
  {
    ...
    bufferByte.resize(recordLength * numTraces);
...
//       float value;
    ...
    for(int i = 0; i < numTraces; i++) {
      ...
      for(int j = 0; j < numSamples; j++) {
        t = (double)j;
        origTraceData[i*numSamples+j] = (float) (cos(omega1*t) * sin(omega2*t));
        testTraceData[i*numSamples+j] = origTraceData[i*numSamples+j];
      }
    }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        testTraceData[i*numSamples+j] = 0.0f;
      }
    }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        if(fabs(testTraceData[i*numSamples+j] - origTraceData[i*numSamples+j]) > 0.025) {
          printf("Error validating compression for trace %d sample=%d  packed[trace][sample]=%g  unpacked[trace][sample]=%g\n",
                 i,j,origTraceData[i*numSamples+j],testTraceData[i*numSamples+j]);
//                  printf("%g, ",origTraceData[i*numSamples+j]-testTraceData[i*numSamples+j]);
        }
      }
    }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        if(fabs(testTraceData[i*numSamples+j] - origTraceData[i*numSamples+j]) > 0.001) {
          printf("Error validating compression for trace %d sample=%d  packed[trace][sample]=%g  unpacked[trace][sample]=%g\n",
                 i,j,origTraceData[i*numSamples+j],testTraceData[i*numSamples+j]);
        }
      }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        origTraceData[i*numSamples+j] = (float)(j * 0.1);
        testTraceData[i*numSamples+j] = origTraceData[i*numSamples+j];
      }
    }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        if(fabs(testTraceData[i*numSamples+j] - origTraceData[i*numSamples+j]) > 1) {
          printf("Error validating compression for trace %d sample=%d  packed[trace][sample]=%g  unpacked[trace][sample]=%g\n",
                 i,j,origTraceData[i*numSamples+j],testTraceData[i*numSamples+j]);
        }
      }
    ...
    for(int i = 0; i < numTraces; i++) {
      for(int j = 0; j < numSamples; j++) {
        origTraceData[i*numSamples+j] = (float)(j * 25.6);
        testTraceData[i*numSamples+j] = origTraceData[i*numSamples+j];
      }
    }
