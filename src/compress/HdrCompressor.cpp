...
bool HdrCompressor::probablyFloat(int iVal) {
  ...
  if(iVal > MIN_POS_FLOAT_BITS  ||  iVal < MAX_NEG_FLOAT_BITS) {
    //       if (isnanf(*(reinterpret_cast<float*>( (int*)&iVal)))) {
    if(isnanf(*(reinterpret_cast<float *>((int *)piVal)))) {
      return false;
    } else {
      return true;
    }
  ...
  while(true) {
    ...
    if(runLength < 3) {
      ...
      if(probablyFloat(firstVal)
        ...
        deltaFloat = *(reinterpret_cast<float *>((int *)&inValues[inCount + 1])) -
                     firstValFloat; //Float.intBitsToFloat(inValues[inCount+1]) - firstValFloat;
