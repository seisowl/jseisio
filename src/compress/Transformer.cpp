...
void Transformer::lotRev8(float *x, int index, int nblocks, float *scratch) {
  ...
  for(i = 0; i < nblocks; i++) {
    ...
    * non-zero. */
    tmp[0] = FILT0 * x[xIndex];
    tmp[1] = FILT8 * x[xIndex];
    tmp[2] = FILT16 * x[xIndex];
    tmp[3] = FILT24 * x[xIndex];
    tmp[4] = FILT32 * x[xIndex];
    tmp[5] = FILT40 * x[xIndex];
    tmp[6] = FILT48 * x[xIndex];
    tmp[7] = FILT56 * x[xIndex];

    if(ISNOTZERO(x[2 + xIndex])) {
      tmp[0] += FILT2 * x[2 + xIndex];
      tmp[1] += FILT10 * x[2 + xIndex];
      tmp[2] += FILT18 * x[2 + xIndex];
      tmp[3] += FILT26 * x[2 + xIndex];
      tmp[4] += FILT34 * x[2 + xIndex];
      tmp[5] += FILT42 * x[2 + xIndex];
      tmp[6] += FILT50 * x[2 + xIndex];
      tmp[7] += FILT58 * x[2 + xIndex];
    }

    if(ISNOTZERO(x[4 + xIndex])) {
      tmp[0] += FILT4 * x[4 + xIndex];
      tmp[1] += FILT12 * x[4 + xIndex];
      tmp[2] += FILT20 * x[4 + xIndex];
      tmp[3] += FILT28 * x[4 + xIndex];
      tmp[4] += FILT36 * x[4 + xIndex];
      tmp[5] += FILT44 * x[4 + xIndex];
      tmp[6] += FILT52 * x[4 + xIndex];
      tmp[7] += FILT60 * x[4 + xIndex];
    }

    if(ISNOTZERO(x[6 + xIndex])) {
      tmp[0] += FILT6 * x[6 + xIndex];
      tmp[1] += FILT14 * x[6 + xIndex];
      tmp[2] += FILT22 * x[6 + xIndex];
      tmp[3] += FILT30 * x[6 + xIndex];
      tmp[4] += FILT38 * x[6 + xIndex];
      tmp[5] += FILT46 * x[6 + xIndex];
      tmp[6] += FILT54 * x[6 + xIndex];
      tmp[7] += FILT62 * x[6 + xIndex];
    }


    /* Ditto for not checking x[1]. */
    tmp[8] = FILT1 * x[1 + xIndex];
    tmp[9] = FILT9 * x[1 + xIndex];
    tmp[10] = FILT17 * x[1 + xIndex];
    tmp[11] = FILT25 * x[1 + xIndex];
    tmp[12] = FILT33 * x[1 + xIndex];
    tmp[13] = FILT41 * x[1 + xIndex];
    tmp[14] = FILT49 * x[1 + xIndex];
    tmp[15] = FILT57 * x[1 + xIndex];

    if(ISNOTZERO(x[3 + xIndex])) {
      tmp[8] += FILT3 * x[3 + xIndex];
      tmp[9] += FILT11 * x[3 + xIndex];
      tmp[10] += FILT19 * x[3 + xIndex];
      tmp[11] += FILT27 * x[3 + xIndex];
      tmp[12] += FILT35 * x[3 + xIndex];
      tmp[13] += FILT43 * x[3 + xIndex];
      tmp[14] += FILT51 * x[3 + xIndex];
      tmp[15] += FILT59 * x[3 + xIndex];
    }

    if(ISNOTZERO(x[5 + xIndex])) {
      tmp[8] += FILT5 * x[5 + xIndex];
      tmp[9] += FILT13 * x[5 + xIndex];
      tmp[10] += FILT21 * x[5 + xIndex];
      tmp[11] += FILT29 * x[5 + xIndex];
      tmp[12] += FILT37 * x[5 + xIndex];
      tmp[13] += FILT45 * x[5 + xIndex];
      tmp[14] += FILT53 * x[5 + xIndex];
      tmp[15] += FILT61 * x[5 + xIndex];
    }

    if(ISNOTZERO(x[7 + xIndex])) {
      tmp[8] += FILT7 * x[7 + xIndex];
      tmp[9] += FILT15 * x[7 + xIndex];
      tmp[10] += FILT23 * x[7 + xIndex];
      tmp[11] += FILT31 * x[7 + xIndex];
      tmp[12] += FILT39 * x[7 + xIndex];
      tmp[13] += FILT47 * x[7 + xIndex];
      tmp[14] += FILT55 * x[7 + xIndex];
      tmp[15] += FILT63 * x[7 + xIndex];
    }

    scratch[15 + scratchIndex] = tmp[0] - tmp[8];
    tmp[8] += tmp[0];
    scratch[14 + scratchIndex] = tmp[1] - tmp[9];
    tmp[9] += tmp[1];
    scratch[13 + scratchIndex] = tmp[2] - tmp[10];
    tmp[10] += tmp[2];
    scratch[12 + scratchIndex] = tmp[3] - tmp[11];
    tmp[11] += tmp[3];
    scratch[11 + scratchIndex] = tmp[4] - tmp[12];
    tmp[12] += tmp[4];
    scratch[10 + scratchIndex] = tmp[5] - tmp[13];
    tmp[13] += tmp[5];
    scratch[9 + scratchIndex]  = tmp[6] - tmp[14];
    tmp[14] += tmp[6];
    scratch[8 + scratchIndex]  = tmp[7] - tmp[15];
    tmp[15] += tmp[7];
...
void Transformer::lotRev16(float *x, int index, int nblocks, float *scratch) {
  ...
  for(i = 0; i < nblocks; i++) {
    ...
    if(ISNOTZERO(x[xIndex])) {
      tmp0 = globalFilt16[0] * x[xIndex];
      tmp2 = globalFilt16[16] * x[xIndex];
      tmp4 = globalFilt16[32] * x[xIndex];
      tmp6 = globalFilt16[48] * x[xIndex];
      tmp8 = globalFilt16[64] * x[xIndex];
      tmp10 = globalFilt16[80] * x[xIndex];
      tmp12 = globalFilt16[96] * x[xIndex];
      tmp14 = globalFilt16[112] * x[xIndex];
      tmp16 = globalFilt16[128] * x[xIndex];
      tmp18 = globalFilt16[144] * x[xIndex];
      tmp20 = globalFilt16[160] * x[xIndex];
      tmp22 = globalFilt16[176] * x[xIndex];
      tmp24 = globalFilt16[192] * x[xIndex];
      tmp26 = globalFilt16[208] * x[xIndex];
      tmp28 = globalFilt16[224] * x[xIndex];
      tmp30 = globalFilt16[240] * x[xIndex];
    } else {
      tmp0 = 0.0F;
      tmp2 = 0.0F;
      tmp4 = 0.0F;
    ...
    if(ISNOTZERO(x[1 + xIndex])) {
      tmp1 = globalFilt16[1] * x[1 + xIndex];
      tmp3 = globalFilt16[17] * x[1 + xIndex];
      tmp5 = globalFilt16[33] * x[1 + xIndex];
      tmp7 = globalFilt16[49] * x[1 + xIndex];
      tmp9 = globalFilt16[65] * x[1 + xIndex];
      tmp11 = globalFilt16[81] * x[1 + xIndex];
      tmp13 = globalFilt16[97] * x[1 + xIndex];
      tmp15 = globalFilt16[113] * x[1 + xIndex];
      tmp17 = globalFilt16[129] * x[1 + xIndex];
      tmp19 = globalFilt16[145] * x[1 + xIndex];
      tmp21 = globalFilt16[161] * x[1 + xIndex];
      tmp23 = globalFilt16[177] * x[1 + xIndex];
      tmp25 = globalFilt16[193] * x[1 + xIndex];
      tmp27 = globalFilt16[209] * x[1 + xIndex];
      tmp29 = globalFilt16[225] * x[1 + xIndex];
      tmp31 = globalFilt16[241] * x[1 + xIndex];
    } else {
      tmp1 = 0.0F;
      tmp3 = 0.0F;
      tmp5 = 0.0F;
    ...
    if(ISNOTZERO(x[2 + xIndex])) {
      tmp0 += globalFilt16[2] * x[2 + xIndex];
      tmp2 += globalFilt16[18] * x[2 + xIndex];
      tmp4 += globalFilt16[34] * x[2 + xIndex];
      tmp6 += globalFilt16[50] * x[2 + xIndex];
      tmp8 += globalFilt16[66] * x[2 + xIndex];
      tmp10 += globalFilt16[82] * x[2 + xIndex];
      tmp12 += globalFilt16[98] * x[2 + xIndex];
      tmp14 += globalFilt16[114] * x[2 + xIndex];
      tmp16 += globalFilt16[130] * x[2 + xIndex];
      tmp18 += globalFilt16[146] * x[2 + xIndex];
      tmp20 += globalFilt16[162] * x[2 + xIndex];
      tmp22 += globalFilt16[178] * x[2 + xIndex];
      tmp24 += globalFilt16[194] * x[2 + xIndex];
      tmp26 += globalFilt16[210] * x[2 + xIndex];
      tmp28 += globalFilt16[226] * x[2 + xIndex];
      tmp30 += globalFilt16[242] * x[2 + xIndex];
    }

    if(ISNOTZERO(x[3 + xIndex])) {
      tmp1 += globalFilt16[3] * x[3 + xIndex];
      tmp3 += globalFilt16[19] * x[3 + xIndex];
      tmp5 += globalFilt16[35] * x[3 + xIndex];
      tmp7 += globalFilt16[51] * x[3 + xIndex];
      tmp9 += globalFilt16[67] * x[3 + xIndex];
      tmp11 += globalFilt16[83] * x[3 + xIndex];
      tmp13 += globalFilt16[99] * x[3 + xIndex];
      tmp15 += globalFilt16[115] * x[3 + xIndex];
      tmp17 += globalFilt16[131] * x[3 + xIndex];
      tmp19 += globalFilt16[147] * x[3 + xIndex];
      tmp21 += globalFilt16[163] * x[3 + xIndex];
      tmp23 += globalFilt16[179] * x[3 + xIndex];
      tmp25 += globalFilt16[195] * x[3 + xIndex];
      tmp27 += globalFilt16[211] * x[3 + xIndex];
      tmp29 += globalFilt16[227] * x[3 + xIndex];
      tmp31 += globalFilt16[243] * x[3 + xIndex];
    }

    if(ISNOTZERO(x[4 + xIndex])) {
      tmp0 += globalFilt16[4] * x[4 + xIndex];
      tmp2 += globalFilt16[20] * x[4 + xIndex];
      tmp4 += globalFilt16[36] * x[4 + xIndex];
      tmp6 += globalFilt16[52] * x[4 + xIndex];
      tmp8 += globalFilt16[68] * x[4 + xIndex];
      tmp10 += globalFilt16[84] * x[4 + xIndex];
      tmp12 += globalFilt16[100] * x[4 + xIndex];
      tmp14 += globalFilt16[116] * x[4 + xIndex];
      tmp16 += globalFilt16[132] * x[4 + xIndex];
      tmp18 += globalFilt16[148] * x[4 + xIndex];
      tmp20 += globalFilt16[164] * x[4 + xIndex];
      tmp22 += globalFilt16[180] * x[4 + xIndex];
      tmp24 += globalFilt16[196] * x[4 + xIndex];
      tmp26 += globalFilt16[212] * x[4 + xIndex];
      tmp28 += globalFilt16[228] * x[4 + xIndex];
      tmp30 += globalFilt16[244] * x[4 + xIndex];
    }

    if(ISNOTZERO(x[5 + xIndex])) {
      tmp1 += globalFilt16[5] * x[5 + xIndex];
      tmp3 += globalFilt16[21] * x[5 + xIndex];
      tmp5 += globalFilt16[37] * x[5 + xIndex];
      tmp7 += globalFilt16[53] * x[5 + xIndex];
      tmp9 += globalFilt16[69] * x[5 + xIndex];
      tmp11 += globalFilt16[85] * x[5 + xIndex];
      tmp13 += globalFilt16[101] * x[5 + xIndex];
      tmp15 += globalFilt16[117] * x[5 + xIndex];
      tmp17 += globalFilt16[133] * x[5 + xIndex];
      tmp19 += globalFilt16[149] * x[5 + xIndex];
      tmp21 += globalFilt16[165] * x[5 + xIndex];
      tmp23 += globalFilt16[181] * x[5 + xIndex];
      tmp25 += globalFilt16[197] * x[5 + xIndex];
      tmp27 += globalFilt16[213] * x[5 + xIndex];
      tmp29 += globalFilt16[229] * x[5 + xIndex];
      tmp31 += globalFilt16[245] * x[5 + xIndex];
    }

    if(ISNOTZERO(x[6 + xIndex])) {
      tmp0 += globalFilt16[6] * x[6 + xIndex];
      tmp2 += globalFilt16[22] * x[6 + xIndex];
      tmp4 += globalFilt16[38] * x[6 + xIndex];
      tmp6 += globalFilt16[54] * x[6 + xIndex];
      tmp8 += globalFilt16[70] * x[6 + xIndex];
      tmp10 += globalFilt16[86] * x[6 + xIndex];
      tmp12 += globalFilt16[102] * x[6 + xIndex];
      tmp14 += globalFilt16[118] * x[6 + xIndex];
      tmp16 += globalFilt16[134] * x[6 + xIndex];
      tmp18 += globalFilt16[150] * x[6 + xIndex];
      tmp20 += globalFilt16[166] * x[6 + xIndex];
      tmp22 += globalFilt16[182] * x[6 + xIndex];
      tmp24 += globalFilt16[198] * x[6 + xIndex];
      tmp26 += globalFilt16[214] * x[6 + xIndex];
      tmp28 += globalFilt16[230] * x[6 + xIndex];
      tmp30 += globalFilt16[246] * x[6 + xIndex];
    }

    if(ISNOTZERO(x[7 + xIndex])) {
      tmp1 += globalFilt16[7] * x[7 + xIndex];
      tmp3 += globalFilt16[23] * x[7 + xIndex];
      tmp5 += globalFilt16[39] * x[7 + xIndex];
      tmp7 += globalFilt16[55] * x[7 + xIndex];
      tmp9 += globalFilt16[71] * x[7 + xIndex];
      tmp11 += globalFilt16[87] * x[7 + xIndex];
      tmp13 += globalFilt16[103] * x[7 + xIndex];
      tmp15 += globalFilt16[119] * x[7 + xIndex];
      tmp17 += globalFilt16[135] * x[7 + xIndex];
      tmp19 += globalFilt16[151] * x[7 + xIndex];
      tmp21 += globalFilt16[167] * x[7 + xIndex];
      tmp23 += globalFilt16[183] * x[7 + xIndex];
      tmp25 += globalFilt16[199] * x[7 + xIndex];
      tmp27 += globalFilt16[215] * x[7 + xIndex];
      tmp29 += globalFilt16[231] * x[7 + xIndex];
      tmp31 += globalFilt16[247] * x[7 + xIndex];
    }

    if(ISNOTZERO(x[8 + xIndex])) {
      tmp0 += globalFilt16[8] * x[8 + xIndex];
      tmp2 += globalFilt16[24] * x[8 + xIndex];
      tmp4 += globalFilt16[40] * x[8 + xIndex];
      tmp6 += globalFilt16[56] * x[8 + xIndex];
      tmp8 += globalFilt16[72] * x[8 + xIndex];
      tmp10 += globalFilt16[88] * x[8 + xIndex];
      tmp12 += globalFilt16[104] * x[8 + xIndex];
      tmp14 += globalFilt16[120] * x[8 + xIndex];
      tmp16 += globalFilt16[136] * x[8 + xIndex];
      tmp18 += globalFilt16[152] * x[8 + xIndex];
      tmp20 += globalFilt16[168] * x[8 + xIndex];
      tmp22 += globalFilt16[184] * x[8 + xIndex];
      tmp24 += globalFilt16[200] * x[8 + xIndex];
      tmp26 += globalFilt16[216] * x[8 + xIndex];
      tmp28 += globalFilt16[232] * x[8 + xIndex];
      tmp30 += globalFilt16[248] * x[8 + xIndex];
    }

    if(ISNOTZERO(x[9 + xIndex])) {
      tmp1 += globalFilt16[9] * x[9 + xIndex];
      tmp3 += globalFilt16[25] * x[9 + xIndex];
      tmp5 += globalFilt16[41] * x[9 + xIndex];
      tmp7 += globalFilt16[57] * x[9 + xIndex];
      tmp9 += globalFilt16[73] * x[9 + xIndex];
      tmp11 += globalFilt16[89] * x[9 + xIndex];
      tmp13 += globalFilt16[105] * x[9 + xIndex];
      tmp15 += globalFilt16[121] * x[9 + xIndex];
      tmp17 += globalFilt16[137] * x[9 + xIndex];
      tmp19 += globalFilt16[153] * x[9 + xIndex];
      tmp21 += globalFilt16[169] * x[9 + xIndex];
      tmp23 += globalFilt16[185] * x[9 + xIndex];
      tmp25 += globalFilt16[201] * x[9 + xIndex];
      tmp27 += globalFilt16[217] * x[9 + xIndex];
      tmp29 += globalFilt16[233] * x[9 + xIndex];
      tmp31 += globalFilt16[249] * x[9 + xIndex];
    }

    if(ISNOTZERO(x[10 + xIndex])) {
      tmp0 += globalFilt16[10] * x[10 + xIndex];
      tmp2 += globalFilt16[26] * x[10 + xIndex];
      tmp4 += globalFilt16[42] * x[10 + xIndex];
      tmp6 += globalFilt16[58] * x[10 + xIndex];
      tmp8 += globalFilt16[74] * x[10 + xIndex];
      tmp10 += globalFilt16[90] * x[10 + xIndex];
      tmp12 += globalFilt16[106] * x[10 + xIndex];
      tmp14 += globalFilt16[122] * x[10 + xIndex];
      tmp16 += globalFilt16[138] * x[10 + xIndex];
      tmp18 += globalFilt16[154] * x[10 + xIndex];
      tmp20 += globalFilt16[170] * x[10 + xIndex];
      tmp22 += globalFilt16[186] * x[10 + xIndex];
      tmp24 += globalFilt16[202] * x[10 + xIndex];
      tmp26 += globalFilt16[218] * x[10 + xIndex];
      tmp28 += globalFilt16[234] * x[10 + xIndex];
      tmp30 += globalFilt16[250] * x[10 + xIndex];
    }

    if(ISNOTZERO(x[11 + xIndex])) {
      tmp1 += globalFilt16[11] * x[11 + xIndex];
      tmp3 += globalFilt16[27] * x[11 + xIndex];
      tmp5 += globalFilt16[43] * x[11 + xIndex];
      tmp7 += globalFilt16[59] * x[11 + xIndex];
      tmp9 += globalFilt16[75] * x[11 + xIndex];
      tmp11 += globalFilt16[91] * x[11 + xIndex];
      tmp13 += globalFilt16[107] * x[11 + xIndex];
      tmp15 += globalFilt16[123] * x[11 + xIndex];
      tmp17 += globalFilt16[139] * x[11 + xIndex];
      tmp19 += globalFilt16[155] * x[11 + xIndex];
      tmp21 += globalFilt16[171] * x[11 + xIndex];
      tmp23 += globalFilt16[187] * x[11 + xIndex];
      tmp25 += globalFilt16[203] * x[11 + xIndex];
      tmp27 += globalFilt16[219] * x[11 + xIndex];
      tmp29 += globalFilt16[235] * x[11 + xIndex];
      tmp31 += globalFilt16[251] * x[11 + xIndex];
    }

    if(ISNOTZERO(x[12 + xIndex])) {
      tmp0 += globalFilt16[12] * x[12 + xIndex];
      tmp2 += globalFilt16[28] * x[12 + xIndex];
      tmp4 += globalFilt16[44] * x[12 + xIndex];
      tmp6 += globalFilt16[60] * x[12 + xIndex];
      tmp8 += globalFilt16[76] * x[12 + xIndex];
      tmp10 += globalFilt16[92] * x[12 + xIndex];
      tmp12 += globalFilt16[108] * x[12 + xIndex];
      tmp14 += globalFilt16[124] * x[12 + xIndex];
      tmp16 += globalFilt16[140] * x[12 + xIndex];
      tmp18 += globalFilt16[156] * x[12 + xIndex];
      tmp20 += globalFilt16[172] * x[12 + xIndex];
      tmp22 += globalFilt16[188] * x[12 + xIndex];
      tmp24 += globalFilt16[204] * x[12 + xIndex];
      tmp26 += globalFilt16[220] * x[12 + xIndex];
      tmp28 += globalFilt16[236] * x[12 + xIndex];
      tmp30 += globalFilt16[252] * x[12 + xIndex];
    }

    if(ISNOTZERO(x[13 + xIndex])) {
      tmp1 += globalFilt16[13] * x[13 + xIndex];
      tmp3 += globalFilt16[29] * x[13 + xIndex];
      tmp5 += globalFilt16[45] * x[13 + xIndex];
      tmp7 += globalFilt16[61] * x[13 + xIndex];
      tmp9 += globalFilt16[77] * x[13 + xIndex];
      tmp11 += globalFilt16[93] * x[13 + xIndex];
      tmp13 += globalFilt16[109] * x[13 + xIndex];
      tmp15 += globalFilt16[125] * x[13 + xIndex];
      tmp17 += globalFilt16[141] * x[13 + xIndex];
      tmp19 += globalFilt16[157] * x[13 + xIndex];
      tmp21 += globalFilt16[173] * x[13 + xIndex];
      tmp23 += globalFilt16[189] * x[13 + xIndex];
      tmp25 += globalFilt16[205] * x[13 + xIndex];
      tmp27 += globalFilt16[221] * x[13 + xIndex];
      tmp29 += globalFilt16[237] * x[13 + xIndex];
      tmp31 += globalFilt16[253] * x[13 + xIndex];
    }

    if(ISNOTZERO(x[14 + xIndex])) {
      tmp0 += globalFilt16[14] * x[14 + xIndex];
      tmp2 += globalFilt16[30] * x[14 + xIndex];
      tmp4 += globalFilt16[46] * x[14 + xIndex];
      tmp6 += globalFilt16[62] * x[14 + xIndex];
      tmp8 += globalFilt16[78] * x[14 + xIndex];
      tmp10 += globalFilt16[94] * x[14 + xIndex];
      tmp12 += globalFilt16[110] * x[14 + xIndex];
      tmp14 += globalFilt16[126] * x[14 + xIndex];
      tmp16 += globalFilt16[142] * x[14 + xIndex];
      tmp18 += globalFilt16[158] * x[14 + xIndex];
      tmp20 += globalFilt16[174] * x[14 + xIndex];
      tmp22 += globalFilt16[190] * x[14 + xIndex];
      tmp24 += globalFilt16[206] * x[14 + xIndex];
      tmp26 += globalFilt16[222] * x[14 + xIndex];
      tmp28 += globalFilt16[238] * x[14 + xIndex];
      tmp30 += globalFilt16[254] * x[14 + xIndex];
    }

    if(ISNOTZERO(x[15 + xIndex])) {
      tmp1 += globalFilt16[15] * x[15 + xIndex];
      tmp3 += globalFilt16[31] * x[15 + xIndex];
      tmp5 += globalFilt16[47] * x[15 + xIndex];
      tmp7 += globalFilt16[63] * x[15 + xIndex];
      tmp9 += globalFilt16[79] * x[15 + xIndex];
      tmp11 += globalFilt16[95] * x[15 + xIndex];
      tmp13 += globalFilt16[111] * x[15 + xIndex];
      tmp15 += globalFilt16[127] * x[15 + xIndex];
      tmp17 += globalFilt16[143] * x[15 + xIndex];
      tmp19 += globalFilt16[159] * x[15 + xIndex];
      tmp21 += globalFilt16[175] * x[15 + xIndex];
      tmp23 += globalFilt16[191] * x[15 + xIndex];
      tmp25 += globalFilt16[207] * x[15 + xIndex];
      tmp27 += globalFilt16[223] * x[15 + xIndex];
      tmp29 += globalFilt16[239] * x[15 + xIndex];
      tmp31 += globalFilt16[255] * x[15 + xIndex];
    }
    ...
    scratch[31 + scratchIndex] = tmp0 - tmp1;
    scratch[1 + scratchIndex] += tmp2 + tmp3;
    scratch[30 + scratchIndex] = tmp2 - tmp3;
    scratch[2 + scratchIndex] += tmp4 + tmp5;
    scratch[29 + scratchIndex] = tmp4 - tmp5;
    scratch[3 + scratchIndex] += tmp6 + tmp7;
    scratch[28 + scratchIndex] = tmp6 - tmp7;
    scratch[4 + scratchIndex] += tmp8 + tmp9;
    scratch[27 + scratchIndex] = tmp8 - tmp9;
    scratch[5 + scratchIndex] += tmp10 + tmp11;
    scratch[26 + scratchIndex] = tmp10 - tmp11;
    scratch[6 + scratchIndex] += tmp12 + tmp13;
    scratch[25 + scratchIndex] = tmp12 - tmp13;
    scratch[7 + scratchIndex] += tmp14 + tmp15;
    scratch[24 + scratchIndex] = tmp14 - tmp15;
    scratch[8 + scratchIndex] += tmp16 + tmp17;
    scratch[23 + scratchIndex] = tmp16 - tmp17;
    scratch[9 + scratchIndex] += tmp18 + tmp19;
    scratch[22 + scratchIndex] = tmp18 - tmp19;
    scratch[10 + scratchIndex] += tmp20 + tmp21;
    scratch[21 + scratchIndex] = tmp20 - tmp21;
    scratch[11 + scratchIndex] += tmp22 + tmp23;
    scratch[20 + scratchIndex] = tmp22 - tmp23;
    scratch[12 + scratchIndex] += tmp24 + tmp25;
    scratch[19 + scratchIndex] = tmp24 - tmp25;
    scratch[13 + scratchIndex] += tmp26 + tmp27;
    scratch[18 + scratchIndex] = tmp26 - tmp27;
    scratch[14 + scratchIndex] += tmp28 + tmp29;
    scratch[17 + scratchIndex] = tmp28 - tmp29;
    scratch[15 + scratchIndex] += tmp30 + tmp31;
    scratch[16 + scratchIndex] = tmp30 - tmp31;
