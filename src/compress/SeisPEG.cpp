...
//     printf("cookie=%d\n",cookie);  	
      if (cookie != COOKIE_V2  &&  cookie != BAD_AMPLITUDE_COOKIE_V2
        ...
        return JS_USERERROR;
          ...
          encodedDataIndex += SIZEOF_SHORT;
          int _idistortion = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex); encodedDataIndex += SIZEOF_INT;
          int _n1 = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex);  encodedDataIndex += SIZEOF_INT;  
          int _n2 = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex);  encodedDataIndex += SIZEOF_INT;  
          int _verticalBlockSize = BlockCompressor::stuffBytesInShort(_encodedData, encodedDataIndex); encodedDataIndex += SIZEOF_SHORT;
          int _horizontalBlockSize = BlockCompressor::stuffBytesInShort(_encodedData, encodedDataIndex); encodedDataIndex += SIZEOF_SHORT;
          int _verticalTransLength = (int)_encodedData[encodedDataIndex];  encodedDataIndex += SIZEOF_CHAR; 
          int _horizontalTransLength = (int)_encodedData[encodedDataIndex]; encodedDataIndex += SIZEOF_CHAR; 
          int _nBytesTraces = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex); encodedDataIndex += SIZEOF_INT;  
          int _nBytesHdrs = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex);  encodedDataIndex += SIZEOF_INT;
          int _iftGainExponent = 0;
          if (cookie == COOKIE_V3  ||  cookie == BAD_AMPLITUDE_COOKIE_V3) {
            _iftGainExponent = BlockCompressor::stuffBytesInInt(_encodedData, encodedDataIndex);  encodedDataIndex += SIZEOF_INT;
          }
