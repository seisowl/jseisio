...
namespace jsIO
{
  enum Attribute { READ_ONLY, READ_WRITE, OVERFLOW_ONLY, RETIRED };
  ...
  class VirtualFolder{
    public:
      ...
      void  addElement(std::string item) {path += "/" + item;};
      void setPath(std::string _path);
      std::string getPath() {return path;};
      Attribute getAttribute() {return attrib;};
      void setAttribute(Attribute _attrib){attrib = _attrib;}
      std::string toString() {return path + "," + attrib2str(attrib);}
...
//    int loadExtents(std::string extBaseName, std::vector<std::string> &extNames, std::vector<long> &extSizes);
      ...
      bool isReadOnly(){return attrib == READ_ONLY;};
      bool isOverFlowOnly() {return attrib == OVERFLOW_ONLY;};
      bool isRetired() {return attrib == RETIRED;};
      bool isWriteable() {return attrib == READ_WRITE;}; 
