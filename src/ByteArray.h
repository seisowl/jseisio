...
namespace jsIO
{
  class ByteArray{
    public:
      ...
      ByteArray(ByteArray const& other){CopyClass(other);}; //Copy constructor
      ...
      const char* getBuffer() const{return (const char*)buffer;};
      unsigned long size() const {return bufsize;};
      unsigned long capacity() const{return buflen;};
...
//    void load(unsigned long pos, char *val , unsigned long valsize);
      ...
      inline char& operator [] (const unsigned long& i){return buffer[i];};
